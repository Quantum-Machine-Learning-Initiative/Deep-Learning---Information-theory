...
def Weight2Matrix3(b, i, N):
    M = sp.sparse.kron(sp.sparse.eye(2**i),sp.sparse.kron(Z, sp.sparse.eye(2**(N-i-1))))
    return b*M
...
def BuildHamiltonian3(W, b, c, N, spins_per_layer):
    H = sp.sparse.csr_matrix(np.zeros([2**N, 2**N]))
    for k,layer in enumerate(W):
        for a,row in enumerate(layer):   
            i = a + int(spins_per_layer[:k].sum()) 
            for e,w in enumerate(row):
                j = e + int(spins_per_layer[:k+1].sum()) 
                M = Weight2Matrix2(w, i, j, N)
                #print(M.diagonal())
                H = np.add(H, M)
    for k, layer in enumerate(b):
        for a, bias in enumerate(layer):
            i = a + int(spins_per_layer[:k+1].sum())
            #print(i)
            #print(bias)
            #print(N)
            M = Weight2Matrix3(bias, i, N)
            #print(M)
            H = np.add(H, M)
        last_layer = k
    for a, bias in enumerate(c[-1]):
        i = a + int(spins_per_layer[:last_layer].sum())
        M = Weight2Matrix3(bias, i, N)
        H = np.add(H, M)          
    return -H


def BuildHamiltonian4(W, b, c, N, spins_per_layer):
    H = sp.sparse.csr_matrix(np.zeros([2**N, 2**N]))
    for k,layer in enumerate(W):
        for a,row in enumerate(layer):   
            i = a + int(spins_per_layer[:k].sum()) 
...
def plot_MI(M_1_i, M_N_i, label):
    ...
    #a.reverse()
    plt.scatter(np.arange(len(M_N_i)).tolist(), M_N_i) 
    plt.savefig('MI_layerN_test_qit.pdf')
    plt.close()
             
def plot_ent(Density_per_layer):
    entropy_per_layer = []
    for rho in Density_per_layer:
        entropy_per_layer.append(Entropy_qit(rho))
    plt.figure(5)
    plt.title('Entropy v/s Layers')
    plt.ylabel('Entropy')
    plt.xlabel('Layers (first to last)')        
    plt.scatter(np.arange(len(entropy_per_layer)).tolist(), entropy_per_layer) 
...
def main():
    ...
    b = np.load('rbm_bias_b.npy')
    c = np.load('rbm_bias_c.npy')
    ...
    H = BuildHamiltonian4(W, b, c, N, spins_per_layer)
    ...
    #print(Density_per_layer[0].data.diagonal().sum())

    
 

    #plot(np.arange(len(Density_per_layer)).tolist(), Density_per_layer)
    ...
    plot_ent(Density_per_layer)

    ploti = Density_per_layer[0].data.diagonal()
    ploti2  = []
    for e in ploti:
        ploti2.append(e) 
    ...
    plt.title('AND Gate ')
    plt.ylabel('P')
    plt.xlabel('spin configurations visible layer')        
    plt.scatter(np.arange(len(ploti)).tolist(), ploti2) 
    plt.savefig('probs-first-layer.pdf')
    plt.close()
